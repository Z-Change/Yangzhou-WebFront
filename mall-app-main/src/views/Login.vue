  ...
  <div class="login">
    ...
    <div v-if="state.type == 'login'" class="login-body login">
      <van-form @submit="onSubmit">
        ...
        <van-field v-model="state.password" type="password" name="password" label="密码"
        ...
        <div style="margin: 16px;">
          ...
          <van-button round block color="#FB7299" native-type="submit">登录</van-button>
        </div>
      </van-form>
    </div>
    <div v-else class="login-body register">
      <van-form @submit="onSubmit">
        <van-field v-model="state.username1" name="username1" label="用户名" placeholder="用户名"
          :rules="[{ required: true, message: '请填写用户名' }]" />
        <van-field v-model="state.password1" type="password" name="password1" label="密码"
          placeholder="密码" :rules="[{ required: true, message: '请填写密码' }]" />
        <van-field center clearable label="验证码" placeholder="输入验证码" v-model="state.verify">
          <template #button>
            <vue-img-verify ref="verifyRef" />
        ...
        <div style="margin: 16px;">
          ...
          <van-button round block color="#FB7299" native-type="submit">注册</van-button>
        </div>
      </van-form>
    </div>
  </div>
