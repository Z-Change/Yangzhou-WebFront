  ...
  <div>
    <header class="home-header wrap" :class="{'active' : state.headerScroll}">
      ...
      <div class="header-search">
        <span class="app-name">动漫周边商城</span>
        <i class="iconfont icon-search"></i>
        <router-link tag="span" class="search-title" to="./product-list?from=home">点我搜索喵❤
        </router-link>
      </div>
    ...
    <div class="category-list">
      <div v-for="item in state.categoryList" v-bind:key="item.categoryId"
    ...
    <div class="good">
      ...
      <van-skeleton title :row="3" :loading="state.loading">
        <div class="good-box">
          <div class="good-item" v-for="item in state.hots" :key="item.goodsId"
            @click="goToDetail(item)">
            <img :src="$filters.prefix(item.goodsCoverImg)" alt="">
            <div class="good-desc">
              <div class="title">{{ item.goodsName }}</div>
...
onMounted(async () => {
  ...
  let items = [];
  for (let i = 0; i < state.swiperList.length; i++) {
