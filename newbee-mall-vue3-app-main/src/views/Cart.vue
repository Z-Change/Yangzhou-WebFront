...
<template>
  <div class="cart-box">
    ...
    <van-submit-bar v-if="state.list.length > 0" class="submit-all van-hairline--top"
      :price="total * 100" button-text="结算" button-type="primary" @submit="onSubmit">
    ...
    <div class="empty" v-if="!state.list.length">
      ...
      <van-button round color="#FB7299" type="primary" @click="goTo" block>前往选购</van-button>
    </div>
    <nav-bar></nav-bar>
  </div>
</template>
...
const onChange = async (value, detail) => {
   ...
   */
  if (
    state.list.find(item => item.cartItemId == detail.name)?.goodsCount == value
  )
    return;
